"""Compare feature reduction with fixed data, budget and two-qubit circuit."""
import argparse
import hashlib
import json
import os
...
import cudaq
from reduction import ROOT, COUNTS, np, load_data, split_data, fit, forward, representation, metrics
from articles.day18.benchmark import search

OUT = ROOT / 'results/day21'
...
def train():
    ...
    protocol = {'source': source, 'task': 'deduplicated binary Iris, 99 rows, Day20 splits',
                ...
                'preprocessing': 'train-only standardization; PCA2 or abs train Pearson top2 or jointly learned tanh bottleneck',
                'quantum': '2 qubits, positive_half angle encoding, one 4-parameter Ansatz, (1-ZZ)/2',
        ...
        for model, count in COUNTS.items():
            ...
            for partition in splits:
                ...
                scores[partition] = {'probabilities': p.tolist(), 'metrics': metrics(labels[partition], p),
                                     ...
                                     'saturated_values_abs_gt_099': int((abs(x) > .99).sum()) if model == 'bottleneck_vqc' else None,
