"""Second real dataset: Wine binary benchmark."""
import argparse
import hashlib
import json
import os
...
import cudaq
from wine_models import ROOT, COUNTS, np, load_data, split_data, fit, forward, representation, metrics
from articles.day18.benchmark import search

OUT = ROOT / 'results/day25'
...
def train():
    ...
    protocol = {'source': source, 'task': 'UCI Wine cultivar 2/3 binary, 119 rows',
                'split_seeds': [2030, 2031], 'initialization_seeds': [42, 43], 'models': COUNTS,
                ...
                'preprocessing': 'train-only raw13 standardization and PCA2 minmax for MLP/VQC/Hybrid; full13 standardized logistic control',
                'quantum': 'Day20 MLP9/VQC4/Hybrid12; VQC and Hybrid two qubits; hybrid head scale initialized .8',
    ...
    for model in selected:
        ...
        for partition, rows in data['splits'].items():
            ...
            records.append({'split_seed': model['split_seed'], 'model': model['model'], 'split': partition,
                            ...
                            'observe_calls': 0 if model['model'] in ('mlp', 'logistic13') else len(rows), 'passed': error < 1e-5})
